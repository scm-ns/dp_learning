...
def AlexNet(features , for_transfer_learning = False):
    ...
    conv1b = tf.Varaible(net_pretrained["conv1"][1])
    ...
    conv2b = tf.Varaible(net_pretrained["conv2"][1])
    ...
    channels = 384
    stride = 1
    conv3_w = tf.Varaible(net_pretrained["conv3"][0])
    conv3_b = tf.Varaible(net_pretrained["conv3"][1])
    ...
    stride = 1
    group = 2
    conv4_w = tf.Varaible(net_pretrained["conv4"][0])
    ...
    stride = 1
    group = 2
    conv5_w = tf.Varaible(net_pretrained["conv5"][0])
    ...
    # fc6
    fc6_w = tf.Varaible(net_pretrained["fc6"][0])
    fc6_b = tf.Varaible(net_pretrained["fc6"][1])
    ...
    fc7_b = tf.Varaible(net_pretrained["fc7"][1])
    fc7 = tf.nn.relu_layer(full_connected6 , fc7W , fc7b)

    if for_transfer_learning:
        return fc7
    
    fc8_w = tf.Varaible(net_pretrained["fc8"][0])
