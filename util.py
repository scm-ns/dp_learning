...
def to_plot(img):
    if K.image_dim_ordering() == "tf":
        ...
        return np.rollaxis(img, 0 , 3).astype(np.unit8)
...
def get_batches(dirname , gen = image.ImageDataGenerator() , shuffle = True , batch_size = 4 , class_mode = "categorical" , target_size = (224 , 224)):
    return gen.flow_from_directory(dirname , target_size = target_size , class_mode = class_mode , shuffle = shuffle , batch_sie = batch_size)
...
def insert_layer(model , new_layer , index):
    res = Sequential()
    for i , layer in enumerate(mode.layers):
        if i == index : 
            res.add(new_layer)
...
def plot_confusion_matrx(cm , classes , normalize = False , title = "Confusion Matrix" , cmap = plt.cm.Blues):
    plt.figure()
    ...
    for i , j in itertools.product(range(cm.shape[0] , range(cm.shape[1])):
            plt.text(j , i , cm[i , j] , horizontalailignment = "center" , color = "white" if cm[i , j] > thresh else "black")
...
def get_classes(path):
    ...
    return (val_batches.classes , batches.classes , onehot(val_batches.classes) , onehot(batches.classes), 
            val_batches.filename , batches.filename , test_batches.filenames)
